...
npm ci         # 커밋된 package-lock.json의 pg, ajv, ajv-formats를 그대로 설치
```

## 환경변수

`.env.example`에는 환경변수 이름과 비밀이 아닌 동결 기본값만 들어 있다. 실제 API 키와
절대 경로는 포함하지 않는다.
...
동봉된 `.env.example`은 평가 프로필을 활성화하고 `DART_QA_CODE_SHA`를 패키지의 런타임
소스 커밋에 고정한다. 선택 기능인 `DART_QA_LATE_EXPANSION`은 채택 모델의 101문항 검증
조건과 동일하게 미설정/OFF 상태로 둔다. 코드 고정값을 평가 결과 커밋으로 바꾸지 않는다.
