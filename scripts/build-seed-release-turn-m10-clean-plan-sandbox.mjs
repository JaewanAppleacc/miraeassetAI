// Turn M10: a SANDBOX-ONLY canonical release manifest + decision pair
// authorizing Plan v0.13 clean candidate (Turn M10's Q18 correction-
// Event connection fix on top of Turn M8's Plan v0.12) for a test-only
// runtime instance -- NOT a v0.20 release manifest/decision, never read
// by domain/runtime/configured-seed-runtime.mjs (untouched, still points
// at v0.19). Reuses structured-artifacts.v0.7.manifest.json UNCHANGED
// (Fact/Coverage/Evidence/Event/Relation/Chain/merged Owner decision --
// none of that changes this Turn) -- only thin_plan/thin_plan_manifest
// differ from Turn M8's own sandbox chain (v0.13 instead of v0.12).
...
const PLAN_V13_PATH = path.join(REPO, "work/domain-seed/seed-thin-flow-plans.v0.13.clean.candidate.jsonl");
const PLAN_V13_MANIFEST_PATH = path.join(REPO, "work/domain-seed/seed-thin-flow-plans.v0.13.clean.candidate.manifest.json");
const GOLD_PATH = path.join(REPO, "work/domain-seed/seed-gold-promotion-candidates.v0.17.jsonl");

const OUT_MANIFEST_PATH = path.join(REPO, "work/domain-seed/seed-release-turn-m10-clean-plan-sandbox.manifest.json");
const OUT_DECISION_PATH = path.join(REPO, "work/domain-seed/seed-release-turn-m10-clean-plan-sandbox.decision.json");

function sha256(bytes) { return createHash("sha256").update(bytes).digest("hex"); }
function fail(msg) { throw new Error(`M10_SANDBOX_RELEASE_BLOCKED: ${msg}`); }
...
async function main() {
  ...
  const planBytes = await readFile(PLAN_V13_PATH);
  const planManifestBytes = await readFile(PLAN_V13_MANIFEST_PATH);
  ...
  if (planRecordCount !== planManifest.record_count) fail(`plan v0.13 record count mismatch`);
  ...
  const manifestWithoutAuth = {
    ...
    release_id: "seed-release-turn-m10-clean-plan-sandbox",
    ...
    supersedes: null,
    supersession_note: "이 파일은 v0.20 release manifest가 아니며 official release 계보에 속하지 않는다. Turn M10의 공통 Composer/Validator/date-role/그룹화 수정 + Plan v0.13(Q18 correction Event 연결)의 Candidate Runtime 통합 검증(wire r14)에서만 사용되는 임시 authorization 체인이다. domain/runtime/configured-seed-runtime.mjs는 이 파일을 참조하지 않으며 계속 v0.19를 가리킨다. Turn M8의 sandbox 체인(seed-release-turn-m8-clean-plan-sandbox.*)과의 유일한 차이는 thin_plan -- v0.12.clean이 아니라 v0.13.clean(Q18 correction Event evidence_id 1건 추가)을 가리킨다.",
  };
  const bindingHash = canonicalManifestBindingHash(manifestWithoutAuth);

  const decision = {
    schema_version: "0.3.0",
    decision_id: "seed-release-turn-m10-clean-plan-sandbox-decision",
    status: "APPROVED",
    ...
    canonical_release_manifest: { path: "work/domain-seed/seed-release-turn-m10-clean-plan-sandbox.manifest.json", sha256: bindingHash },
    ...
    thin_plan: { path: "work/domain-seed/seed-thin-flow-plans.v0.13.clean.candidate.jsonl", sha256: sha256(planBytes), record_count: planRecordCount },
    thin_plan_manifest: { path: "work/domain-seed/seed-thin-flow-plans.v0.13.clean.candidate.manifest.json", sha256: sha256(planManifestBytes) },
    ...
    basis: "Turn M10 공통 Composer/Validator/Flow 수정 + Plan v0.13(Q18 기존 VERIFIED correction Event 연결). Sandbox 전용 -- 공식 release 계보 아님, v0.20 아님.",
  };
  ...
  const finalManifest = {
    ...
    release_authorization: {
      ...
      decision_artifact_path: "work/domain-seed/seed-release-turn-m10-clean-plan-sandbox.decision.json",
