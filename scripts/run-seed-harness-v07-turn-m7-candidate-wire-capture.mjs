// Turn M7 Section 6: the SAME versioned wire-capture logic as
// scripts/run-seed-harness-v07-synthesis-sandbox-wire-capture-versioned.mjs
// (reused verbatim -- runHarness, Gold, metric evaluation, diff-vs-prior-
// revision), with exactly ONE difference: it spawns the Turn M7 Candidate
// SANDBOX entrypoint (scripts/start-agent-server-turn-m7-candidate-
// sandbox.mjs, which never touches domain/runtime/configured-seed-runtime.mjs)
// instead of the production scripts/start-agent-server.mjs. determineRevisions()
// is untouched, so it naturally picks the next integer after the highest
// existing seed-harness-v07-wire.r* directory (r11, since r1-r10 already
// exist and are never modified here).
...
const PORT = Number(process.env.SEED_HARNESS_PORT ?? 8745);
...
const LIFECYCLE_PATH = path.join(REPO, "work/domain-seed/seed-harness-v07-turn-m7-synthesis-sandbox-lifecycle.sandbox-only.json");
const LEDGER_PATH = path.join(REPO, "work/domain-seed/seed-harness-v07-turn-m7-synthesis-sandbox-usage-ledger.sandbox-only.jsonl");
...
async function determineRevisions() {
  ...
  if (highestExisting === 0) throw new Error("expected r1-r10 to already exist -- refusing to run as if this were the first-ever capture");
...
function startServer() {
  const child = spawn(process.execPath, [path.join(REPO, "scripts/start-agent-server-turn-m7-candidate-sandbox.mjs")], {
...
async function main() {
  ...
  console.error(`Turn M7 Candidate sandbox run (v07, wire r11): release_eligible=${sourceTree.release_eligible} (expected false -- this is a Candidate integration test, never a release).`);
  ...
  const config = {
    ...
    run_id: `seed-harness-v07-turn-m7-candidate-wire-capture-r${newRevision}-${randomUUID()}`,
  };
  ...
  try {
    ...
    await writeFile(path.join(wireDir, "index.json"), `${JSON.stringify({ generated_at: new Date().toISOString(), revision: newRevision, count: wireIndex.length, entries: wireIndex, note: "Turn M7 Candidate sandbox integration test wire -- NOT an official release wire, NOT scored, sourced from seed-release-turn-m7-candidate-sandbox.manifest.json." }, null, 2)}\n`, "utf8");
  ...
  const receipt = {
    ...
    candidate_authorization: { manifest: "work/domain-seed/seed-release-turn-m7-candidate-sandbox.manifest.json", note: "SANDBOX-only authorization chain, never a v0.20 release" },
    summary,
