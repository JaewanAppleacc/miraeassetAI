// Turn M4 Section 7: a NEW revision of the Candidate review UI (never
// overwrites data-candidates-v0.1). Shows all 8 Candidate Facts on one
// screen -- the 7 that PASSED Turn M4's independent DocumentIR
// re-verification unchanged from v0.8, plus the 1 that had a genuine
// defect (fact_74a2b743fee3b410295be924's raw_label) shown with its
// v0.9-corrected value and an explicit note of WHAT was wrong and why.
// Same review mechanics as v0.1: APPROVE/FIX_REQUIRED/REJECT, all
// starting PENDING, no AI auto-approval. Read-only over the Candidate/
// verification artifacts -- never modifies them.
...
const VERIFICATION_REPORT_PATH = path.join(REPO, "work/handoff/seed-final-response-owner-review/results/seed-structured-gap-candidate-verification-report.v0.1.json");
const EVIDENCE_VERIFIED_PATH = path.join(REPO, "work/domain-seed/seed-evidence-verified.v0.9.jsonl");
const OUT_DIR = path.join(REPO, "work/handoff/seed-final-response-owner-review/ui/data-candidates-v0.2");
...
const FINAL_EXPORT_FILENAME = "seed-structured-gap-candidate-owner-decision.v0.2.jsonl";
...
async function main() {
  ...
  const verificationReport = JSON.parse(await readFile(VERIFICATION_REPORT_PATH, "utf8"));
  const findingByFactId = new Map(verificationReport.findings.map((f) => [f.fact_id, f]));

  if (v08Rows.length !== 8) throw new Error(`BLOCKER: expected 8 rows in v0.8 delta, found ${v08Rows.length}`);

  const records = v08Rows.map((v08Fact) => {
    const supersede = v09ByFactId.get(v08Fact.fact_id);
    const fact = supersede ?? v08Fact;
    const finding = findingByFactId.get(v08Fact.fact_id);
    ...
    return {
      ...
      raw_label_source: supersede ? "v0.9 (Turn M4 correction)" : "v0.8 (Turn M3, unchanged)",
      ...
      turn_m4_verdict: finding?.verdict ?? null,
      turn_m4_real_row_label: finding?.real_row_label ?? null,
      turn_m4_note: finding?.note ?? null,
      turn_m4_semantic_fit_caveat: fact.attributes.review_provenance.semantic_fit_caveat ?? null,
    };
  });
  ...
  const payload = {
    ...
    fact_candidates_v08_path: "work/domain-seed/seed-facts-candidates.v0.8.delta.jsonl",
    fact_candidates_v08_sha256: sha256(v08Bytes),
    fact_candidates_v09_path: "work/domain-seed/seed-facts-candidates.v0.9.delta.jsonl",
    fact_candidates_v09_sha256: sha256(v09Bytes),
    verification_report_path: "work/handoff/seed-final-response-owner-review/results/seed-structured-gap-candidate-verification-report.v0.1.json",
    verification_report_sha256: sha256(await readFile(VERIFICATION_REPORT_PATH)),
  ...
  const report = {
    ...
    corrected_record_count: records.filter((r) => r.raw_label_source.startsWith("v0.9")).length,
    final_export_filename: FINAL_EXPORT_FILENAME,
  };
  await writeFile(OUT_REPORT_PATH, `${JSON.stringify(report, null, 2)}\n`, "utf8");
  console.log(JSON.stringify({ output_html_sha256: report.output_html_sha256, record_count: records.length, corrected_record_count: report.corrected_record_count }, null, 2));
}
...
function buildHtml(payload) {
  ...
  return [
    ...
    "<title>Turn M4 구조화 Candidate 검수 (v0.2)</title>",
...
const CSS_TEXT = [
  ...
  ".card.corrected { border-left: 4px solid var(--warn); }",
  ...
  ".m4-note-box { white-space: pre-wrap; background:#fff7ed; border:1px solid #fed7aa; border-radius:6px; padding:10px; font-size: 12px; line-height:1.5; margin: 8px 0; }",
...
"  var STORAGE_KEY = 'seed-structured-gap-candidate-review-v0.2-state-' + DATA.source_decision_sha256;",
"",
...
"      text('h1', null, 'Turn M4 \\uAD6C\\uC870\\uD654 Candidate \\uAC80\\uC218 v0.2 (' + IDS.length + '\\uAC74)'),",
"      text('div', 'meta-line', 'v0.8: ' + DATA.fact_candidates_v08_path + ' | v0.9 delta: ' + DATA.fact_candidates_v09_path),",
...
"    draftBtn.addEventListener('click', function () { downloadText('seed-structured-gap-candidate-review-v02-draft.jsonl', buildExportLines()); });",
...
"    var record = BY_ID[id]; var entry = reviewState[id];",
"    var isCorrected = record.raw_label_source.indexOf('v0.9') === 0;",
"    var card = el('div', { className: 'card' + (isCorrected ? ' corrected' : '') });",
...
"    if (isCorrected) {",
"      card.appendChild(el('div', { className: 'm4-note-box' }, [text('b', null, 'Turn M4 \\uC218\\uC815\\uB428: '), document.createTextNode('raw_label\\uC774 \\uC2E4\\uC81C \\uBB38\\uC11C \\uD14D\\uC2A4\\uD2B8\\uC640 \\uC77C\\uCE58\\uD558\\uC9C0 \\uC54A\\uC544 v0.9\\uB85C \\uC218\\uC815\\uB428. \\uC2E4\\uC81C row label: ' + (record.turn_m4_real_row_label || '-') + '. ' + (record.turn_m4_note || ''))]));",
"      if (record.turn_m4_semantic_fit_caveat) card.appendChild(el('div', { className: 'm4-note-box' }, [text('b', null, '\\uC758\\uBBF8 \\uC801\\uD569\\uC131 \\uC720\\uBCF4: '), document.createTextNode(record.turn_m4_semantic_fit_caveat)]));",
"    } else {",
"      card.appendChild(el('div', { className: 'basis-box' }, [text('b', null, 'Turn M4 \\uC7AC\\uAC80\\uC99D: '), document.createTextNode((record.turn_m4_verdict || '-') + ' -- ' + (record.turn_m4_note || ''))]));",
"    }",
...
"    card.appendChild(text('h4', null, 'Candidate Fact (' + record.raw_label_source + ')'));",
