...
//
// PERIOD-SEMANTICS FIX (superseding scripts/build-seed-fact-batch-v06-candidates.mjs
// for the two Q10 CONSOLIDATION_ENTITY_COUNT facts only): v0.6's Q10 facts
// were period_type:"CUMULATIVE" with period_start/period_end spanning a
// half-year (e.g. 2023-01-01..2023-06-30) and as_of_date set to the
// SUBMISSION date (2023-08-14) -- but "연결에 포함된 회사수" (consolidation
// scope headcount) is a point-in-time snapshot as of the reporting date,
// not a cumulative flow over the half-year, and as_of_date was conflating
// "when the count is true" with "when it was disclosed". Fixed to
// period_type:"POINT_IN_TIME", as_of_date = the reporting date the count
// actually describes (2023-06-30 / 2025-06-30, period_start/period_end
// null per the POINT_IN_TIME contract), with known_at/valid_from kept
// separately as the real submission date (2023-08-14 / 2025-08-14) via a
// new, decoupled `knownAt` parameter on buildCandidateFact -- this exact
// as_of_date/known_at split already exists elsewhere in the real VERIFIED
// data (e.g. fact_e5ad6617ee0e6287fb222ce3, ACQUISITION_RETIREMENT_STATUS:
// as_of_date "2025-06-30", known_at/valid_from "2025-08-14T00:00:00Z"), so
// this is a return to the codebase's own established convention, not a
// new one. The disclosed numbers (10 / 16) and every Evidence field
// (evidence_id/source_locator/quoted_text/quote_sha256) are UNCHANGED --
// only the two Facts' period fields move.
//
// P0 AUDIT FIX (carried over from v06, unchanged): this script authors
// CANDIDATE-only records. verification_status:"CANDIDATE" throughout,
// owner_disposition:"PENDING", no reviewer, no approval literal anywhere
// in this file's code. See scripts/promote-seed-fact-batch-v06.mjs for the
// SEPARATE promotion script that requires a real, externally authored
// decision artifact before anything is promoted to VERIFIED (it works
// unchanged against whichever candidate/decision-template revision is
// pointed at it via its options).
//
// New files only, all CANDIDATE/PENDING: seed-evidence-candidates.v0.9.delta.jsonl,
// seed-facts-candidates.v0.7.delta.jsonl, seed-structured-owner-decision.v0.7-batch.template.jsonl.
// scripts/build-seed-fact-batch-v06-candidates.mjs and its output
// (seed-evidence-candidates.v0.8.delta.jsonl, seed-facts-candidates.v0.6.delta.jsonl,
// seed-structured-owner-decision.v0.6-batch.template.jsonl) are preserved
// untouched -- this script neither reads nor writes them.
...
const EVIDENCE_CANDIDATES_PATH = path.join(REPO, "work/domain-seed/seed-evidence-candidates.v0.9.delta.jsonl");
const FACT_CANDIDATES_PATH = path.join(REPO, "work/domain-seed/seed-facts-candidates.v0.7.delta.jsonl");
const DECISION_TEMPLATE_PATH = path.join(REPO, "work/domain-seed/seed-structured-owner-decision.v0.7-batch.template.jsonl");
...
// knownAt is now independent of asOfDate: as_of_date describes WHEN the
// value is true (the reporting/period date), known_at/valid_from describe
// WHEN it became known (the disclosure/submission date) -- for most of
// this batch's facts those coincide (a same-day exchange/major filing), so
// callers that omit knownAt keep the prior fallback (asOfDate, then
// generatedAt's date) unchanged. Only Q10's two facts pass a distinct
// knownAt below.
function buildCandidateFact({ corpCode, sourceDocumentId, metricCode, rawLabel, valueType, normalizedValue, rawValueText, unit, scale, scope, periodType, periodStart, periodEnd, asOfDate, knownAt, evidenceId, generatedAt }) {
  const factId = ids.fact(corpCode, metricCode, asOfDate ?? "no-period", scope, sourceDocumentId);
  const knownAtDate = knownAt ?? asOfDate ?? generatedAt.slice(0, 10);
  const fact = {
    fact_id: factId,
    corp_code: corpCode,
    event_id: null,
...
export async function buildCandidates({ generatedAt = new Date().toISOString() } = {}) {
  ...
  // --- Q10: consolidation_entity_count 2023/2025 --------------------
  // Fixed period semantics (see header comment): POINT_IN_TIME as of the
  // reporting date the count describes, known/valid from the actual
  // submission date. Evidence unchanged from v06.
  ...
  const factQ10Before = buildCandidateFact({
    ...
    scope: "CONSOLIDATED", periodType: "POINT_IN_TIME", asOfDate: "2023-06-30", knownAt: "2023-08-14",
  ...
  const factQ10After = buildCandidateFact({
    ...
    scope: "CONSOLIDATED", periodType: "POINT_IN_TIME", asOfDate: "2025-06-30", knownAt: "2025-08-14",
  ...
  const decisionTemplate = facts.map((f, i) => ({
    review_item_id: `structured_review_v07_batch_${f.fact_id.slice(5, 15)}`,
    fact_id: f.fact_id,
    ...
    reviewer: null,
    reviewed_at: null,
    notes: "Codex 독립 검수(v0.17 감사)에서 발견된 metric_fail 원인 조사 중 식별된 신규 Fact 후보. Q10 두 건은 기간 의미 결함(POINT_IN_TIME/as_of_date/known_at 분리) 수정판. 이 템플릿은 코드가 생성했으며 owner_disposition/reviewer/reviewed_at은 사람이 직접 채워야 한다.",
  }));
