// Turn M3 item 11: builds a NEW, fully offline, single-file HTML
// before/after review UI for the SAME 24 FIX_REQUIRED items, reading the
// NEW matrix v0.3 (r8 before / r9 after) -- never modifies the matrix,
// the decision, or the existing v0.1/v0.2/v0.3 UI files. Output is two
// brand-new files under
// work/handoff/seed-final-response-owner-review/ui/v0.4/.
//
// Carries forward v0.3's semantic diff + remaining-gap surfacing, updated
// to Turn M3 item 12's status vocabulary (RESOLVED/IMPLEMENTATION_REQUIRED/
// CANDIDATE_DATA_REVIEW_REQUIRED/PLAN_CANDIDATE_REVIEW_REQUIRED/
// ONTOLOGY_PROPOSAL_REQUIRED/SOURCE_DATA_NOT_AVAILABLE/
// OWNER_POLICY_DECISION_REQUIRED -- no more PARTIAL/BLOCKED, per Turn M3's
// explicit correction that every remaining item must be a concretely
// actionable next step). Every item starts PENDING; nothing is pre-approved.
...
const MATRIX_PATH = path.join(REPO, "work/handoff/seed-final-response-owner-review/results/seed-response-remediation-matrix.v0.3.jsonl");
...
const WIRE_BEFORE_DIR = path.join(REPO, "work/domain-seed/seed-harness-v07-wire.r8");
const WIRE_AFTER_DIR = path.join(REPO, "work/domain-seed/seed-harness-v07-wire.r9");
const OUT_DIR = path.join(REPO, "work/handoff/seed-final-response-owner-review/ui/v0.4");
...
const FINAL_EXPORT_FILENAME = "seed-response-remediation-owner-decision.v0.3.jsonl";
const STATUS_LIST = ["RESOLVED", "IMPLEMENTATION_REQUIRED", "CANDIDATE_DATA_REVIEW_REQUIRED", "PLAN_CANDIDATE_REVIEW_REQUIRED", "ONTOLOGY_PROPOSAL_REQUIRED", "SOURCE_DATA_NOT_AVAILABLE", "OWNER_POLICY_DECISION_REQUIRED"];
...
async function main() {
  ...
  for (const row of matrixRows) {
    ...
    records.push({
      ...
      turn_m_status: row.turn_m2_status,
      status: row.status,
      remediation_item_ids: row.turn_m3_remediation_item_ids,
  ...
  const payload = {
    ...
    source_matrix_path: "work/handoff/seed-final-response-owner-review/results/seed-response-remediation-matrix.v0.3.jsonl",
    ...
    before_wire_revision: "r8",
    after_wire_revision: "r9",
...
const CSS_TEXT = [
  ...
  ".status-badge.CANDIDATE_DATA_REVIEW_REQUIRED { background:#fee2e2; color:#9a3412; }",
  ".status-badge.SOURCE_DATA_NOT_AVAILABLE { background:#e5e7eb; color:#374151; }",
...
"  var STORAGE_KEY = 'seed-remediation-review-v0.4-state-' + DATA.source_matrix_sha256;",
"",
...
"",
"  var STATUS_FILTERS = ['ALL', 'RESOLVED', 'IMPLEMENTATION_REQUIRED', 'CANDIDATE_DATA_REVIEW_REQUIRED', 'PLAN_CANDIDATE_REVIEW_REQUIRED', 'ONTOLOGY_PROPOSAL_REQUIRED', 'SOURCE_DATA_NOT_AVAILABLE', 'OWNER_POLICY_DECISION_REQUIRED', 'PENDING'];",
...
"    draftBtn.addEventListener('click', function () { downloadText('seed-response-remediation-review-draft.v0.3.jsonl', buildExportLines()); });",
