// Turn M10 Section 13: a NEW revision of the Turn M9 final-integration
// review UI (final-integration-v0.1 is left completely untouched -- this
// writes to a separate final-integration-v0.2 directory). Same 6-question
// review surface and embedded data as v0.1; the only functional change is
// the export mechanism:
//   - Draft/FINAL export now trigger a REAL file download (Blob +
//     URL.createObjectURL, revoked immediately after triggering the
//     download) with the correct filename, in addition to the existing
//     textarea (kept as a fallback/inspection copy).
//   - A "클립보드에 복사" button copies the exported text.
//   - A success message confirms the download/copy actually happened.
//   - On load, if a v0.1 localStorage entry for the SAME packet SHA
//     exists (same-origin file:// sharing is browser-dependent, so this
//     is opportunistic, never assumed), a banner offers to import it --
//     never silently overwrites v0.2's own state, never forces
//     re-judgment. A manual "v0.1 export JSONL 붙여넣기로 가져오기"
//     textarea is also provided as a robust fallback when localStorage is
//     not shared across the two file:// documents.
// The Turn M9 Owner decision has ALREADY been extracted and preserved
// (see seed-v020-final-integration-owner-decision.v0.1.jsonl) -- this
// revision exists purely to prevent the SAME export gap from recurring in
// future reviews, never to demand the Owner re-judge anything already
// decided.
...
const OUT_DIR = "work/handoff/seed-final-response-owner-review/ui/final-integration-v0.2";
...
const V01_HTML_PATH = "work/handoff/seed-final-response-owner-review/ui/final-integration-v0.1/seed-v020-final-integration-owner-review.html";
...
// Turn M9's own build script re-embeds a fresh generated_at timestamp
// into final-integration-v0.1's HTML on every rebuild (including
// whenever tests/seed-final-integration-owner-review-ui.test.mjs's own
// test.before re-runs it) -- its SHA is therefore NOT a fixed historical
// constant safe to hardcode here. This script reads it dynamically at
// build start and re-reads it after writing v0.2's own output, so the
// ONE safety property that actually matters -- this v0.2 build never
// mutates v0.1 -- is still verified, without being brittle to some other
// process's legitimate, unrelated rebuild of v0.1 in between.
...
function fail(msg) { throw new Error(`M10_UI_V02_BUILD_BLOCKED: ${msg}`); }
...
async function main() {
  ...
  const v01HtmlShaBefore = sha256(await readAbs(V01_HTML_PATH));
  ...
  if (extra.length > 0) fail(`integration packet has unexpected extra question(s): ${extra.join(",")}`);
  ...
  const packetSha = PINNED_INPUTS["work/handoff/seed-final-response-owner-review/results/seed-response-turn-m8-integration-packet.v0.1.json"];
  const payload = {
    ...
    integration_packet_sha256: packetSha,
  ...
  for (const rel of Object.keys(PINNED_INPUTS)) {
    if (inputHashesBefore[rel] !== inputHashesAfter[rel]) fail(`${rel}: sha256 changed during build`);
  }
  const v01HtmlShaAfter = sha256(await readAbs(V01_HTML_PATH));
  if (v01HtmlShaBefore !== v01HtmlShaAfter) fail(`${V01_HTML_PATH}: sha256 changed during THIS build -- v0.2's own build must never mutate v0.1`);
  ...
  const report = {
    ...
    v01_html_path: V01_HTML_PATH,
    v01_html_sha256_at_build_time: v01HtmlShaAfter,
    v01_html_unchanged_by_this_build: v01HtmlShaBefore === v01HtmlShaAfter,
...
<head>
<meta charset="utf-8">
<title>Turn M8 v0.20-r2 Candidate 최종 통합 검수 v0.2</title>
...
<header id="app-header">
  <h1>v0.20-r2 Candidate 최종 통합 검수 v0.2 (실제 다운로드/복사 지원)</h1>
  ...
  <p class="hint">이 페이지는 완전 오프라인이며, 판정은 이 브라우저의 localStorage에만 저장됩니다. 자동 승인은 없습니다. Turn M9 FINAL Owner 판정(6/6 FIX_REQUIRED)은 이미 seed-v020-final-integration-owner-decision.v0.1.jsonl로 추출·보존되었습니다 -- 이 v0.2는 향후 회귀 방지용입니다.</p>
  <div id="migration-banner" hidden></div>
</header>
<main id="app"></main>
<section id="import-panel">
  <h2>v0.1 export JSONL 붙여넣기로 가져오기 (localStorage가 공유되지 않을 때의 대체 수단)</h2>
  <textarea id="import-input" rows="6" placeholder="v0.1에서 export한 JSONL을 여기 붙여넣고 가져오기를 누르세요"></textarea>
  <button id="import-btn" type="button">가져오기</button>
  <span id="import-message" role="status"></span>
</section>
...
#migration-banner { margin-top: 8px; padding: 8px 10px; background: #1e3a8a; border-radius: 6px; font-size: 13px; }
#migration-banner button { margin-left: 8px; padding: 4px 10px; border-radius: 4px; border: none; background: #fff; color: #1e3a8a; cursor: pointer; }
...
#import-panel, #export-panel { max-width: 1100px; margin: 0 auto 24px; padding: 0 16px; }
#import-panel textarea, #export-output { width: 100%; font-family: monospace; font-size: 12px; }
#export-actions { display: flex; align-items: center; gap: 10px; margin: 8px 0; }
#export-actions button, #import-btn { padding: 6px 14px; border-radius: 6px; border: 1px solid var(--border); background: #fff; cursor: pointer; }
#export-message, #import-message { font-size: 13px; color: var(--ok); }
`;
...
(function () {
  ...
  function stateFromStoredObject(parsed) {
    var out = {};
  ...
  function loadState() {
    try {
      ...
      if (raw) return stateFromStoredObject(JSON.parse(raw));
    } catch (e) { /* corrupted localStorage -- fall back to defaults, never throw */ }
    return stateFromStoredObject(null);
  }
  var state = loadState();
  function saveState() {
  ...
  // ---- v0.1 -> v0.2 migration (opportunistic + manual fallback) --------
  // Same-origin file:// localStorage sharing across two different HTML
  // documents is browser-dependent -- this NEVER assumes it works. If it
  // does (same key formula as v0.1), a banner OFFERS the import; it never
  // silently overwrites v0.2's own already-in-progress state.
  function hasAnyJudgement(s) {
    return payload.records.some(function (r) { return s[r.question_id].owner_disposition !== "PENDING"; });
  }
  (function offerAutoMigrationIfAvailable() {
    if (hasAnyJudgement(state)) return; // v0.2 already has real judgements -- never overwrite
    try {
      var raw = window.localStorage.getItem(STORAGE_KEY);
      if (!raw) return;
      var candidate = stateFromStoredObject(JSON.parse(raw));
      if (!hasAnyJudgement(candidate)) return;
      var banner = document.getElementById("migration-banner");
      banner.hidden = false;
      banner.textContent = "이 브라우저에 기존 판정 데이터가 있습니다. ";
      var btn = el("button", { type: "button" }, [txt("가져오기")]);
      btn.addEventListener("click", function () {
        state = candidate; saveState(); banner.hidden = true; renderAll();
      });
      banner.appendChild(btn);
    } catch (e) { /* ignore -- manual paste-import below still works */ }
  })();

  function importFromExportText(text) {
    var lines = text.split("\n").map(function (l) { return l.trim(); }).filter(function (l) { return l.length > 0; });
    var imported = {};
    lines.forEach(function (line) {
      var rec;
      try { rec = JSON.parse(line); } catch (e) { return; }
      if (rec && rec.question_id) {
        imported[rec.question_id] = {
          owner_disposition: rec.owner_disposition || "PENDING", reviewer: rec.reviewer || null,
          reviewed_at: rec.reviewed_at || null, notes: rec.notes || null,
          information_limit_accepted: rec.information_limit_accepted != null ? rec.information_limit_accepted : null,
          checklist: {},
        };
        if (rec.checklist_results) {
          var kinds = ["common", "specific"];
          kinds.forEach(function (kind) {
            var arr = rec.checklist_results[kind];
            if (Array.isArray(arr)) {
              imported[rec.question_id].checklist[kind] = {};
              arr.forEach(function (item, idx) { imported[rec.question_id].checklist[kind][idx] = !!item.checked; });
            }
          });
        }
      }
    ...
    if (count === 0) return 0;
    payload.records.forEach(function (r) {
      if (imported[r.question_id]) state[r.question_id] = Object.assign(defaultRecordState(), imported[r.question_id]);
    });
    saveState();
    return count;
  }
  ...
  function renderChecklist(items, kind, qid) {
    var ul = el("ul", { className: "checklist" });
    items.forEach(function (itemText, idx) {
      ...
      var input = el("input", { type: "checkbox", id: checkboxId });
  ...
  function renderCard(record) {
    ...
    var s = state[qid];
    var badgeDisposition = el("span", { className: "badge badge-disposition-" + s.owner_disposition }, [txt(s.owner_disposition)]);
    ...
    var header = el("div", { className: "card-header" }, [el("h2", null, [txt(qid)]), badgeDisposition, badgeStatus, badgePass]);
    ...
    var errorBox = el("div", { className: "judgement-error" });
    var infoLimitCheckbox = null;
    if (qid === "question_seed_v07_18") {
      var gate = el("div", { className: "info-limit-gate" });
      infoLimitCheckbox = el("input", { type: "checkbox", id: "info-limit-accepted-" + qid });
      ...
      gate.appendChild(el("label", { for: "info-limit-accepted-" + qid }, [txt(" 발행총액 정보한계 문구를 승인합니다 -- APPROVE_RESPONSE 선택 전 필수")]));
      body.appendChild(gate);
    }
    var notes = el("textarea", { className: "notes", id: "notes-" + qid, placeholder: "notes (선택)" });
    notes.value = s.notes || "";
    ...
    DISPOSITIONS.forEach(function (disp) {
      ...
      btn.addEventListener("click", function () {
        ...
        if (qid === "question_seed_v07_18" && disp === "APPROVE_RESPONSE") {
          ...
          if (!accepted) { errorBox.textContent = "Q18은 information_limit_accepted 체크 없이 APPROVE_RESPONSE 할 수 없습니다."; return; }
  ...
  function doExport(mode) {
    ...
    try {
      ...
      var text = lines.join("\n");
      var filename = mode === "final" ? payload.final_export_filename : "seed-v020-final-integration-owner-decision.draft.jsonl";
      panel.hidden = false;
  ...
  document.getElementById("copy-btn").addEventListener("click", function () {
    ...
    function fallbackCopy() {
      ...
      catch (e) { msg.textContent = "복사에 실패했습니다 -- 직접 선택해 복사해주세요."; }
    }
    if (navigator.clipboard && navigator.clipboard.writeText) {
      navigator.clipboard.writeText(lastExportText).then(function () {
  ...
  document.getElementById("import-btn").addEventListener("click", function () {
    ...
    var count = importFromExportText(input.value);
    if (count === 0) { msg.textContent = "가져올 유효한 레코드를 찾지 못했습니다."; return; }
    msg.textContent = count + "건을 가져왔습니다.";
    renderAll();
  });
  ...
  document.getElementById("reset-btn").addEventListener("click", function () {
    ...
    state = stateFromStoredObject(null);
    renderAll();
  });

  window.__seedFinalIntegrationReviewV02 = {
    buildExportLines: buildExportLines, buildRecord: buildRecord, getState: function () { return state; },
    importFromExportText: importFromExportText,
  };
