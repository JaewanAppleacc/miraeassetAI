// Turn M8 Section 7: the SAME versioned wire-capture logic as Turn M7's
// run-seed-harness-v07-turn-m7-candidate-wire-capture.mjs, spawning the
// Turn M8 clean-Plan sandbox entrypoint instead. Writes to
// seed-harness-v07-wire.r12/ -- r10/r11 are never touched.
...
const PORT = Number(process.env.SEED_HARNESS_PORT ?? 8746);
...
const LIFECYCLE_PATH = path.join(REPO, "work/domain-seed/seed-harness-v07-turn-m8-synthesis-sandbox-lifecycle.sandbox-only.json");
const LEDGER_PATH = path.join(REPO, "work/domain-seed/seed-harness-v07-turn-m8-synthesis-sandbox-usage-ledger.sandbox-only.jsonl");
...
async function determineRevisions() {
  ...
  if (highestExisting < 11) throw new Error("expected r1-r11 to already exist -- refusing to run as if this were an early capture");
...
function startServer() {
  const child = spawn(process.execPath, [path.join(REPO, "scripts/start-agent-server-turn-m8-clean-plan-sandbox.mjs")], {
...
async function main() {
  ...
  console.error(`Turn M8 clean-Plan Candidate sandbox run (v07, wire r12): release_eligible=${sourceTree.release_eligible} (expected false).`);
  ...
  const config = {
    ...
    run_id: `seed-harness-v07-turn-m8-clean-plan-wire-capture-r${newRevision}-${randomUUID()}`,
  };
  ...
  try {
    ...
    await writeFile(path.join(wireDir, "index.json"), `${JSON.stringify({ generated_at: new Date().toISOString(), revision: newRevision, count: wireIndex.length, entries: wireIndex, note: "Turn M8 clean-Plan Candidate sandbox integration test wire -- NOT an official release wire, NOT scored, sourced from seed-release-turn-m8-clean-plan-sandbox.manifest.json (v0.6-based clean Plan, no sub_request_authority)." }, null, 2)}\n`, "utf8");
  ...
  const receipt = {
    ...
    candidate_authorization: { manifest: "work/domain-seed/seed-release-turn-m8-clean-plan-sandbox.manifest.json", note: "SANDBOX-only authorization chain, clean v0.6-based Plan, never a v0.20 release" },
    summary,
