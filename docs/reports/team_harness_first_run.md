...
## 새 질문 24문항 — LLM까지 켠 최종 답변 실측 (2026-08-31)

지금까지 새 질문은 검색·근거까지만 쟀다(LLM 끔). 이번엔 끝까지: HCX-005 문항당 1회, 총 22호출(2건은 계산기가 처리해 호출 불필요), 134.31원.

**정답형 20문항**: 완전 정답 15, 부분 정답 3(금액은 맞고 기간·만기일 누락), 오답 2.
오답 2건(N21, N24)은 둘 다 같은 회사의 다른 시점 공시/행을 고른 것 — 지어낸 게 아니라
선택 오류. 섹션 선택 병목과 같은 뿌리다.
(N09는 채점 문자열이 "계약이행 불가" vs 답변 "계약이행이 불가"로 한 글자 어긋나
부분으로 집계됐지만 내용은 완전 정답 — 실질 완전 정답 16.)

**함정 4문항**: N14(범위 밖) 깨끗한 거부. N13(없는 기업) "기업을 특정하지 못했다"
경고와 함께 인접 공시만 나열 — 지어내지 않음. N17(모호 질문) 경고는 달았지만 숫자
하나를 제시 — 절반 방어, 약점으로 기록. N15는 함정 설계가 빗나감 — "코퍼스에 없다"고
만든 질문인데 실제로 임원 학력이 코퍼스에 있었고 시스템이 그 원문을 제시했다.

**지어낸 숫자: 0건 / 24문항.** 검증: SUPPORTED 13, PARTIALLY_SUPPORTED 11.
JSON 파싱 실패 3건은 전부 근거 나열 폴백으로 강등 — 죽지 않음.

누적 실호출 비용: 약 531원(VAT 별도) / 예산 3,000원.
