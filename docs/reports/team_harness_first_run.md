...
## Phase 1 DEV_TUNE 101문항 — 검색·근거 전수 (2026-09-01, LLM 미사용)

재완이 공개한 Gold Phase1 DEV_TUNE 101문항(207 중 봉인분 106 제외). 정답 문서 106건을
DocumentIR에서 뽑아 캐시 구축 후 LLM 없이 전수. 비용 0원.

| 지표 | 수리 전 | 수리 후 |
|---|---|---|
| 정답 문서 상위20 (92문항) | 90/92 = 97.8% | **92/92 = 100%** |
| 근거 슬롯 상위20 (270슬롯) | 244 = 90.4% | **251 = 93.0%** |
| 근거 슬롯 인용 | 184 = 68.1% | **199 = 73.7%** |
| 함정 9문항 (NOT_FOUND 5 + WITHHELD 4) | 1/9 | **9/9** |
| 정답형 92문항 오발동 | — | 0 |

수리 3종(전부 dart_detective 층, 검색 코어 불변):
1. 하이픈 날짜("2024-04-17")를 연도 조건으로 읽고 그 날 접수된 문서를 Stage1 절단 앞으로
   당긴다(+청크 가산 0.3). 놓쳤던 삼성E&A·신한지주 2건 회복.
2. "코퍼스에 포함되어 있는가" 유형은 문서 인덱스를 기업·유형·연도로 세서 0건이면 NOT_FOUND.
3. 공시유보: 유보사유 칸/본문 문장을 찾고, 질문이 그 값을 물을 때만 WITHHELD. 유보 행이
   상위 청크 밖이어도 근거 문서 전체 행을 본다.

회귀: gold25 E-R@20 0.9429 (+0.0000), new24 근거 26/36, gold25 커버리지 47/140 — 전부 불변.
테스트 747 passed. CLOSED 66문항 문서 100%, OPEN 26문항 문서 100%.

## Phase 1 DEV_TUNE 101문항 — LLM 켜고 끝까지 (2026-09-01, run B)

HCX-005 문항당 1회, 66호출(35문항은 계산기·규칙이 답해 호출 생략), **346.92원**(VAT 별도).
이후 수리는 전부 저장된 원문 응답을 재파싱한 재채점(호출 0회, 0원).

| 지표 (정답형 92문항) | run B 직후 | 수리 후 재채점 |
|---|---|---|
| status 일치 (101문항: SUPPORTED/NOT_FOUND/WITHHELD) | 101/101 | **101/101** |
| 정답값 완전 포함 | 49 | **63** |
| 부분 포함 | 28 | 18 |
| 미포함 | 15 | 11 |
| CLOSED 완전 | 38/66 | **52/66** |
| OPEN 완전 | 11/26 | 11/26 |
| 지어낸 숫자 | 0 | **0** |

수리(전부 결정론, 프롬프트 불변):
1. answer가 항목별 dict로 온 13문항 — 값은 다 맞았는데 빈 답으로 버려짐 → "키: 값" 직렬화
2. "얼마나 변동했는가" 12문항이 계산기를 못 깨움 → 트리거에 "변동" 추가
3. 모델이 document_id를 "[doc_id]" 대괄호째 베껴 "문서 불일치"로 5문항 폐기 → 괄호만 벗김
4. 대량보유 "변화" 트리거, 원 단위 증감액 백만원 환산(팀 Gold 표기)
5. 표 인용이 머리글+행 합쳐진 꼴("계약기간 | 종료일 | 2029-12-31") → 셀마다 원문 그대로면
   soft 인정. 셀 하나라도 없으면 그대로 폐기

남은 것: LLM 인용문이 원문을 풀어 쓴 15건(숫자는 전부 원문 일치)은 현행 규칙상 폐기.
정책 결정 사항으로 남긴다. 유형별 완전 정답: 수치조회 47/55, 서술형 10/20, 비교계산 5/12,
사건추적 1/5. 누적 실호출 비용 약 878원 / 3,000원.
