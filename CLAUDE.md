...
- **재검수 4차 반영(2026-09-05 · 0e294ed · judge33)**: 코덱스 4차(배포 보류 — BLOCKER 2·HIGH 1·WARN 1)
  전건 타당 판정·반영. [B1] 미해소 복수 후보 차단을 렌더링 성공과 분리(unresolved_multi_doc 불리언) —
  분리 답변은 항상 생성(후보 전부 블록, 값 미추출 항목 '확인 불가', 문서의 모든 항목 행 스캔; 머리글/값
  분리 표 재현 잠금). [B2] 추론형 요구(영향·전망·평가·의미·추이)는 원문에 그 단어 문장이 없으면 LLM 없이
  "확인할 수 없다"로 결정론 종료, 있거나 텍스트형(목적·사유·상태·인원…)이면 LLM 답 문장이 그 단어를 담은
  인용 quote에 결박돼야 채택(narrative_unbound) — 기존 EchoLLM 테스트를 음성으로 전환. [H3] 고유 토큰
  해소 실패 시 정확 일자 후보 우선(±1일은 정확 후보 없을 때만), 분리 답변 머리말 '같은 날짜'/'전후 하루
  범위' 구분·계약명/계약상대 텍스트 식별자(접수번호·'1일' 같은 숫자는 validator 오폭이라 미사용).
  **judge33(보충 ON·k40 OFF): 완전 56·부분 30·미포함 6·가중 71.0·답변가능성 101/101(전건 일치, 최초)**,
  judge32 대비 1↑/1↓ PASS · narrative_unbound 실발동 2·items_ambiguous_docs 4·보충 16 · gold25 r7
  full 4/partial 16/zero 5 · 테스트 849·안전세트 28/28. 실코퍼스: KF-21 06-26 정확 일자 해소→결정론.
  잔여(정직): 날짜에 '일'이 없는 항목 질문(gold25 Q01 "2023년 4월")은 후보 결박이 없어 발췌 덤프에 다른
  계약 행이 섞일 수 있음(월 단위 결박 미구현) · EVENT_TRACE 1/5(서식 파서 미구현).
- **B안 최소 범위(2026-09-05 · f5b4d6f · judge34)**: 월 단위 서식 항목 결박 + 확정 슬롯 문장 렌더러(코덱스
  합의 범위 — 별칭 사전·LLM 재호출·인용 수리·ResolvedFact/7패턴 리팩터·프롬프트 변경 없음). '일' 없는
  연·월 질문은 그 달 접수 공시로 항목 자리 결박(정확 일자 창 우선, 해소 규칙 기존 그대로: 고유 토큰 →
  후보 1건 → 아니면 분리 답변), 결박 시 evidence·context·sources·retrieved_context 전부 그 문서로.
  렌더러: "해당 공시에서 확인한 계약금액은 X원이며, 계약기간은 A부터 B까지입니다." + 근거 목록(값 원문
  그대로·단위는 라벨 괄호). 관측: state.binding → think_trace 'binding'·meta.binding. **judge34(보충 ON·
  k40 OFF): 완전 60·부분 26·미포함 6·가중 73.0·답변가능성 101/101(최고 경신)**, judge33 대비 개선 5·악화
  1(효성중공업 — 정확 일자 경로, j31~j34 full/partial 교대의 FC 변동, 변경 무관) PASS · gold25 r8: Q01·Q02
  partial→full(full 6·zero 5), Q01은 4월 문서만 결박·타 계약 값 혼입 0 · 5-string 계약 위반 0/126 ·
  테스트 859·안전세트 28/28. 월 결박 발동은 gold25 Q01·Q02 2문항, DEV_TUNE 101에는 해당 문형 없음.
