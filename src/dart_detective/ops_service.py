"""팀원2 운영 계약 서버 — interfaces.md §2 그대로.

배포 스켈레톤이자 최종 서빙 앱이다. 답을 만드는 쪽(§2-1 `answer_api.answer`)이
아직 없으면 더미 5필드로 돌고(오늘의 공개망 스켈레톤), `dart_detective/answer_api.py`가
생기는 순간 자동으로 실물에 연결된다 — 이 파일은 고칠 필요가 없다.

이 모듈이 소유하는 것(§2-2):
  - GET /answer 파라미터 검증(누락·빈 값·중복 -> 400), 5필드 그대로 전달
  - 캐시: key = question_id + sha256(question) + 설정 지문(readiness().pins)
          폴백·degraded 결과는 캐시 금지
...
def _dummy_answer(question_id: str, question: str, *,
                  deadline_s: float | None = None) -> dict[str, str]:
    """스켈레톤 더미 — 계약 모양(5개 str)만 실물과 동일하다."""
    return {
...
def _load_provider() -> tuple[Callable[..., dict], Callable[[], dict]]:
    try:
        from . import answer_api  # type: ignore  # 지영(에이전트 층) 산출물
        return answer_api.answer, answer_api.readiness
    except Exception:  # noqa: BLE001 — 없으면 스켈레톤 모드
        return _dummy_answer, _dummy_readiness


_answer, _readiness = _load_provider()


def configure(answer_fn: Callable[..., dict] | None = None,
              readiness_fn: Callable[[], dict] | None = None) -> None:
    """테스트·수동 배선용. None이면 다시 자동 탐색."""
    global _answer, _readiness
    if answer_fn is None and readiness_fn is None:
        _answer, _readiness = _load_provider()
        return
...
def cacheable(response: Mapping[str, str]) -> bool:
    """폴백·degraded 결과는 캐시하지 않는다(§2-2) — 나쁜 답의 박제 방지.

    판정은 think_trace의 표식으로 한다: '"fallback": true' 또는 degraded 모드 표식.
    실물 answer_api가 이 표식을 남기는 것이 계약이고, 더미는 항상 degraded다.
    """
    trace = response.get("think_trace", "")
    if '"fallback": true' in trace:
        return False
    if '"mode": "dummy"' in trace or '"degraded"' in trace:
        return False
    try:
...
app = FastAPI(title="DART QA ops", version="1.0")
...
async def _startup() -> None:
    ...
    try:
        ...
        await asyncio.to_thread(_answer, "warmup-0", "워밍업 질문입니다.",
                                deadline_s=30.0)
...
async def answer_endpoint(request: Request) -> JSONResponse:
    ...
    async with _gate:
        ...
        task = asyncio.create_task(
            asyncio.to_thread(_answer, question_id, question, deadline_s=remaining))
        try:
            while True:
            ...
            response = {
                ...
                "retrieved_context": "",
                "think_trace": json.dumps({"fallback": True, "reason": "deadline"},
                                          ensure_ascii=False),
            ...
            response = {
                "question_id": question_id, "question": question,
                "retrieved_context": "",
                "think_trace": json.dumps({"fallback": True,
                                           "reason": type(exc).__name__},
                                          ensure_ascii=False),
                "answer": "내부 오류로 답변을 생성하지 못했습니다.",
            }

    response = {k: str(response.get(k, "")) for k in WIRE_KEYS}
    stored = cacheable(response)
    if stored:
        _cache_store(key, response)
    logger.info("answer cache=%s qid=%s chars=%d cached=%s ms=%d",
                "miss", question_id, len(question), stored,
